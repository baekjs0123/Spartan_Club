    ...
    for i in range(N):          # 행 갯수만큼 반복
        for j in range(M):      # 열 갯수만큼 반복
            ...
            for k in range(4): # 델타배열탐색을 위해 방향 4번 돌린다.
                for l in range(1, distance + 1): # 거리는 1부터 distance까지
                    ni = i + di[k] * l # 다음 위치는 현재 위치 + 델타배열 방향 * 디스턴스 거리만큼 계속 더해준다.
                    nj = j + dj[k] * l # 다음 위치는 현재 위치 + 델타배열 방향 * 디스턴스 거리만큼 계속 더해준다.
                    if is_valid(): # 유효성 검사 통과하면 카운트에 꽃가루 수를 더해준다.
                        cnt += balloons[ni][nj]
            # 만약 max_cnt < cnt가 크면
            if max_cnt < cnt:
